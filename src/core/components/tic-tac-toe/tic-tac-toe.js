...
const TicTacToe = () => {
    ...
    let [state, dispatch] = useReducer(ticTacToeReducer, initialState);

    function markPosition(i, j) {
        dispatch({type: 'mark-position', payload: {i: i, j: j}});
    }
    ...
    function getWinnerDisplayImage(winner) {
        const imgWinner = <img className="mx-auto" src={winnerCongratulationsImage} ></img>;
        const imgDraw = <img className="mx-auto" width="300px" src={drawImage}></img>
        ...
        <div>
            ...
            <div className="flex  w-1/6 h-20 mx-auto ">
                <div className="ttt-box w-1/3 border-r-1 border-b-2 border-black text-6xl text-center" onClick={() => markPosition(0,0)} >
                    <OXComponent markType={state.matrix[0][0]} currentPlayer={state.currentPlayer}></OXComponent>                        
                </div>
                <div className="w-1/3 border-r-2 border-b-2 border-l-2 border-black " onClick={() => markPosition(0,1)} >
                    <OXComponent markType={state.matrix[0][1]} currentPlayer={state.currentPlayer}></OXComponent>                        
                </div>
                <div className="w-1/3 border-b-2 border-l-1 border-black " onClick={() => markPosition(0,2)}>
                    <OXComponent markType={state.matrix[0][2]} currentPlayer={state.currentPlayer}></OXComponent>     
                </div>
            </div>
            <div className="flex  w-1/6 h-20 mx-auto ">
                <div className="w-1/3 border-r-1 border-b-2 border-black " onClick={() => markPosition(1,0)}>
                    <OXComponent markType={state.matrix[1][0]} currentPlayer={state.currentPlayer}></OXComponent>     
                </div>
                <div className="w-1/3 border-r-2 border-b-2 border-l-2 border-black " onClick={() => markPosition(1,1)}>
                    <OXComponent markType={state.matrix[1][1]} currentPlayer={state.currentPlayer}></OXComponent>     
                </div>
                <div className="w-1/3 border-b-2 border-l-1 border-black " onClick={() => markPosition(1,2)}>
                    <OXComponent markType={state.matrix[1][2]} currentPlayer={state.currentPlayer}></OXComponent>     
                </div>
            </div>
            <div className="flex  w-1/6 h-20 mx-auto ">
                <div className="w-1/3 border-r-1 border-black " onClick={() => markPosition(2,0)}>
                    <OXComponent markType={state.matrix[2][0]} currentPlayer={state.currentPlayer}></OXComponent>     
                </div>
                <div className="w-1/3 border-r-2 border-l-2 border-black " onClick={() => markPosition(2,1)}>
                    <OXComponent markType={state.matrix[2][1]} currentPlayer={state.currentPlayer}></OXComponent>     
                </div>
                <div className="w-1/3 border-l-1 border-black " onClick={() => markPosition(2,2)}>
                    <OXComponent markType={state.matrix[2][2]} currentPlayer={state.currentPlayer}></OXComponent>
                </div>
            </div>
            
            {getWinnerDisplayText(state.winner)}
            
            <div className={state.winnerDiagonal} ></div>
