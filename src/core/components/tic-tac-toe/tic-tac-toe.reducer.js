...
function markPosition(state, i, j) {
    ...
    if(state.matrix[i][j] === 0) {
        let matrixUpdated = [...state.matrix];
...
function determineWinnerOrDraw(matrix) {
    ...
    for(let i=0; i < winnerPositions.length; i++) {
        let sequence3Winner = winnerPositions[i];

        let sequenceWinner = [];
        for(let j=0; j < 2; j++) {
            let row = sequence3Winner[j][0];
            let column = sequence3Winner[j][1];

            let rowNext = sequence3Winner[j+1][0];
            let columnNext = sequence3Winner[j+1][1];
